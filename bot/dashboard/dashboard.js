...
app.use(session({
  ...
  store :MongoStore.create({ mongoUrl: jsonconfig.MONGOURL})
}));
...
var url = req.protocol + '://' + req.get('host') + req.originalUrl;
    ...
    app.get("/support", (req, res) => {
      res.redirect(`https://discord.gg/3PGCdhTn4k`)
    });
...
res.redirect(`https://discord.com/api/oauth2/authorize?client_id=892340813906968587&permissions=8&redirect_uri=https%3A%2F%2Fbotdashboard.minhducgaming.ihostfull.com%2Fthanks&scope=bot%20applications.commands`)
});
...
channel.send(embed)
 ...
 app.get("/redeem", checkAuth, (req, res) => {
    ...
    app.get("/dashboard", checkAuth, (req, res) => {
      ...
      const server = client.guilds.cache.get('870197933197369394');
...
if(config.datadogApiKey){
  metrics.init({ apiKey: jsonconfig.datadogApiKey, host: '', prefix: 'hey.' });
