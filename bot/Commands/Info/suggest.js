...
module.exports = {
  name: "suggest",
  description: "Suggest Features for NMDGTV",
  botPerms: ["EMBED_LINKS"],
  run: async (client, message, args) => {
    const avatar = message.author.avatarURL;
    const suggestchannel = client.channels.cache.get("898830962497052682");
    const suggestion = args.join(" ");
    if (!suggestion) {
      return message.channel.send("Please Suggest Something");
    }
    message.channel.send(
      "Thanks for Suggesting Features for NMDGTV. Our Mod team will inform you if your Suggestion is accepted or not!"
    );
